...
# ------------------------------------------------------------
# 1. READ DATASET (Dynamic Path Resolution)
# ------------------------------------------------------------
...
# Automatically create the 'dataset' directory if it does not exist
...
# dropna() removes values like NaN or None from data.
...
deletion_indicator = (df_original.isnull().any(axis=1)).astype(int)
...
for column in numeric_columns:
    if df_mean[column].isnull().any():
        # calculates the average value of all numbers in a specific column
        mean_value = df_mean[column].mean()
        # this calculates the mean, excluding missing values, to fill empty spots
...
for column in numeric_columns:
    if df_median[column].isnull().any():
        # calculates the middle value (median) of a specific column
        median_value = df_median[column].median()

        # replaces missing values with the column's statistical median
    ...
    for target_column in numeric_columns:
        ...
        for column in numeric_columns:
            if column != target_column:
                ...
                if df_model[column].notnull().sum() >= 2:
        ...
        # ----------------------------------------------------
        # Calculate linear regression manually (y = mx + b)
        # ----------------------------------------------------
        x_mean = x.mean()
        y_mean = y.mean()
        ...
        if len(missing_rows) > 0:
            predicted_values = (
                slope * missing_rows[predictor_column] + intercept
            )

            df_model.loc[missing_rows.index, target_column] = predicted_values

            print("\nTarget column:", target_column)
            print("Predictor column:", predictor_column)
            print("Slope:", slope)
            print("Intercept:", intercept)
            print("Number of values predicted:", len(missing_rows))
...
for column in numeric_columns:
    if df_model[column].isnull().any():
        ...
        df_model[column] = df_model[column].fillna(median_value)
...
for column in categorical_columns:
    if df_model[column].isnull().any():
        ...
        if len(mode_values) > 0:
            df_model[column] = df_model[column].fillna(mode_values.iloc[0])
...
for column in df_original.columns:
    indicator_column = column + "_Missing"
...
final_result["Deletion_Row_Removed"] = deletion_indicator

# B. Mean-imputed values
for column in numeric_columns:
    final_result[column + "_Mean_Imputed"] = df_mean[column]

# C. Median-imputed values
for column in numeric_columns:
    final_result[column + "_Median_Imputed"] = df_median[column]

# D. Model-based imputed values
for column in numeric_columns:
    final_result[column + "_Model_Imputed"] = df_model[column]

# E. Missing Indicator features
for column in df_original.columns:
    final_result[column + "_Missing"] = (
        df_original[column].isnull()
    ).astype(int)
...
print(
    "\nAll missing-value handling techniques " "are stored in ONE CSV file."
)